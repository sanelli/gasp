...
shared_ptr<language::blaise_statement_compund>gasp::blaise::language::make_compound_statement()
{
...
shared_ptr<blaise_statement> gasp::blaise::language::make_assignement_statement(unsigned int line, unsigned int column,
                                                                  shared_ptr<blaise_generic_memory_location> variable,
                                                                  shared_ptr<blaise_expression> expression) {
   if(variable->variable_type() == blaise_variable_type::CONSTANT)
      throw blaise_language_error(line, column, make_string("Variable '", variable->name() ,"' is constant."));

   if(expression->result_type() != variable->type() && !language::blaise_language_utility::can_cast(expression->result_type(), variable->type())){
        throw blaise_language_error(line, column, make_string("Cannot cast '", expression->result_type(), "' into '",variable->type(),"'."));
     }
