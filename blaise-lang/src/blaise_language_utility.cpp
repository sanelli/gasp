...
bool blaise_language_utility::is_numeric(blaise_expression_type type){
   switch(type){
...
bool blaise_language_utility::is_boolean(blaise_expression_type type){
   switch(type){
...
bool blaise_language_utility::is_string(blaise_expression_type type){
   switch(type){
...
bool blaise_language_utility::is_char(blaise_expression_type type){
   switch(type){
...
blaise_expression_type blaise_language_utility::get_binary_numeric_result(blaise_expression_type left, blaise_expression_type right){
   if(!is_numeric(left))
      throw std::runtime_error("left operand is not a numeric type");
   if(!is_numeric(right))
      throw std::runtime_error("right operand is not a numeric type");

   if(left == blaise_expression_type::INTEGER && right == blaise_expression_type::INTEGER)
      return blaise_expression_type::INTEGER;
   if((left == blaise_expression_type::FLOAT && right == blaise_expression_type::FLOAT) ||
      (left == blaise_expression_type::INTEGER && right == blaise_expression_type::FLOAT) ||
      (left == blaise_expression_type::FLOAT && right == blaise_expression_type::INTEGER))
      return blaise_expression_type::FLOAT;
   if(left == blaise_expression_type::DOUBLE || right == blaise_expression_type::DOUBLE)
      return blaise_expression_type::DOUBLE;

   throw std::runtime_error("Unexpected runtime combination");
}

blaise_expression_type blaise_language_utility::get_binary_boolean_result(blaise_expression_type left, blaise_expression_type right){
   if(!is_boolean(left))
      throw std::runtime_error("left operand is not a boolean type");
   if(!is_boolean(right))
      throw std::runtime_error("right operand is not a boolean type");

   if(left == blaise_expression_type::BOOLEAN && right == blaise_expression_type::BOOLEAN)
      return blaise_expression_type::BOOLEAN;
   throw std::runtime_error("Unexpected runtime combination");
}

blaise_expression_type blaise_language_utility::get_binary_char_result(blaise_expression_type left, blaise_expression_type right){
   if(!is_char(left))
      throw std::runtime_error("left operand is not a char type");
   if(!is_char(right))
      throw std::runtime_error("right operand is not a char type");

   if(left == blaise_expression_type::CHAR && right == blaise_expression_type::CHAR)
      return blaise_expression_type::CHAR;
   throw std::runtime_error("Unexpected runtime combination");
}

blaise_expression_type blaise_language_utility::get_binary_string_result(blaise_expression_type left, blaise_expression_type right){
   ...
   if(left == blaise_expression_type::STRING || right == blaise_expression_type::STRING)
      return blaise_expression_type::STRING;
   throw std::runtime_error("Unexpected runtime combination");
}

blaise_expression_type blaise_language_utility::get_resulting_type(const token<blaise_token_type>& token_operator, blaise_expression_type operand) {
...
blaise_expression_type blaise_language_utility::get_resulting_type(const token<blaise_token_type>& token_operator, blaise_expression_type left, blaise_expression_type right) {
   switch(token_operator.type()) {
